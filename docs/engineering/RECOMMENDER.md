...
MEETABLE は名前のある施設ノードに限る。通路上の無名ノードは候補にしない。各地点に `evidence`: `hypothesis` | `field_confirmed` を付ける。仮説を実測として返さない。
...
**改札は路線ごとに 1 つでは足りない。** JR 新宿は降りた車両の位置で使う改札が変わる。改札が変われば各人の距離が変わり、候補の順位まで変わる。代表 1 つに丸めると、そこが丸ごと嘘になる。

| catalogId | 代表ケース |
|---|---|
| `entry.jr.west` | JR 西口改札 |
| `entry.jr.south` | JR 南口改札 |
| `entry.jr.east` | JR 東口改札 |
| `entry.jr.newsouth` | JR 新南口改札 |
| `entry.keio.west` | 京王線 西口改札 |
| `entry.keio.main` | 京王線 中央口改札 |
| `entry.marunouchi.west` | 丸ノ内線 西口方面改札 |
| `entry.marunouchi.east` | 丸ノ内線 東口方面改札 |
| `entry.odakyu.west` | 小田急 西口改札 |
| `entry.oedo.main` | 都営大江戸線 改札 |

実際の改札名と数は ZIP 取り込みで確定する。`barrier:gate` と名前のあるノードから起こす。ここに挙げたのは形の例であって、確定した一覧ではない。
...
出口ノードの緯度経度は都データから作る。手順は `docs/research/RESEARCH.md`。`mx` / `my` は HERE のメルカトルなので、図面のアフィン `d[].t` とコミュニティの `ref_frame.transform` で変換する。
