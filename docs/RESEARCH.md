...
出口に着いたら [Maps URLs の Directions](https://developers.google.com/maps/documentation/urls/get-started#directions-action) を開く。`origin` は出口、`destination` は目的地、`travelmode` は `walking`。Routes API は使わない。契約は `docs/RECOMMENDER.md` の `mapsDirUrl`。
